# OCP Helpdesk cover email — v5 DRAFT
...
I'm writing to introduce **independent implementation research** I've been undertaking on what an OCDS-aligned publication of New Zealand government procurement data could look like in practice. The work is not affiliated with any government agency, vendor, or institution — it's a civil-society-oriented research project examining whether NZ's OGP commitment **NZ0029** (*Improve Government Procurement Transparency*) can be substantively delivered against publicly available data, ahead of its current 2030 timeline.
...
I'd like OCP's technical review on the mapping and the sample output before this work is engaged with MBIE / NZGP or published more broadly. Specifically, I have **two practical asks** and seven points where independent technical view would strengthen the work.

## Two practical asks

1. **Prefix registration.** The DRT flags the OCID prefix `ocds-nz-gets-` as unregistered. I'd like to formally register it. What's the process, is `ocds-nz-gets` available, and what publisher metadata is required?
...
5. **NZ extensions — 18 proposed.** Each anchored to Rules + glossary. Particularly value feedback on:
   ...
   The pipeline currently carries `agencyTier` and `abbreviation` on `parties[].details` as the seeds of `nz-agency-tier`. The DRT correctly flags these as not-in-standard. Once a registered prefix and your view on the extension proposals are in hand, I'll formalise schemas in the [extensions registry](https://github.com/open-contracting-extensions) format.
...
A 30-minute video call would be ideal but not essential. Written feedback on the spreadsheet and sample release would be enormously helpful — particularly on the seven points above. I'm happy to share further details of the pipeline implementation if that would aid review.
...
**v5 changes from v4:**
- **All Project Unify references removed** — first person "I" / "this research" throughout.
- **All Palantir Foundry references removed** — pipeline described in platform-neutral terms ("the pipeline", "automated regeneration", "output-to-source lineage").
- **Reframed as "independent implementation research"** — sets the tone in the opening sentence. Not vendor-positioned, not government-affiliated, not adversarial activist. Solo researcher with a deliverable.
- **Subject line updated** to lead with "Independent research."
- **"We're operating outside government" paragraph rewritten** as "The work is **independent and unfunded**. I'm a solo researcher. There is no commercial interest behind this email, no vendor positioning, and no government affiliation."

**Follow-up needed (not blocking this email):**
- The GitHub repo `README.md` mentions Project Unify / Foundry — needs updating to match the research framing.
- `CITATION.cff` lists `affiliation: Project Unify` — change to remove or rename.
- The sample release JSON has `"publisher": { "name": "Project Unify (test)" }` — update to "NZ-OCDS Research (test)" or similar before attaching.

**Carry-forward notes:**
- Lead with the Rules-OCDS link
- NSW failure mode acknowledged respectfully
- OCP Helpdesk typically responds in 1–3 weeks
